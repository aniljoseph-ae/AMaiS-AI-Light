...
'''class InteractiveChatApp:
    ...
    def setup_vectorstore(self):
        ...
        This will be loaded lazily from session_state to avoid redundant loading.
        """
        # Check if vectorstore is already cached in session_state
        if "vectorstore" not in st.session_state:
            embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/paraphrase-MiniLM-L6-v2")
            st.session_state.vectorstore = Chroma(persist_directory="chatLLM_vector_store", embedding_function=embeddings)
